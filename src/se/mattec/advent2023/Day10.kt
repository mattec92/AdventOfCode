...
object Day10 {
    ...
    fun problem1(): String {
        val pipes = data.mapIndexed { y: Int, s: String -> s.mapIndexed { x: Int, c: Char -> c.toPipePart(x, y) } }
        val startY = pipes.indexOfFirst { it.any { it?.isStart == true } }
        val startX = pipes[startY].indexOfFirst { it?.isStart == true }
        ...
        do {
            ...
            val deltaX = if (currentPipe.isStart) {
                currentPipe.horizontalConnections.firstOrNull() ?: 0
            } else {
                currentPipe.horizontalConnections.firstOrNull { it != -(lastDeltaX) } ?: 0
            }
            val deltaY = if (currentPipe.isStart) {
                if (deltaX == 0) {
                    currentPipe.horizontalConnections.firstOrNull() ?: 0
                } else {
                    0
                ...
                currentPipe.verticalConnections.firstOrNull { it != -(lastDeltaY) } ?: 0
            }
            x += deltaX
            y += deltaY
            lastDeltaX = deltaX
        ...
        return (steps / 2.0).roundToInt().toString()
    }

    private val BOTH_CONNECTIONS = setOf(1, -1)
    ...
    fun problem2(): String {
        ...
        val mainLoop = mutableSetOf<Pair<Int, Int>>().apply { add(startX to startY) }
        val left = mutableSetOf<Pair<Int, Int>>()
        ...
        do {
            ...
            val deltaY = if (currentPipe.isStart) {
                if (deltaX == 0) {
                    currentPipe.verticalConnections.firstOrNull() ?: 0
                } else {
                    0
                }
            } else {
            ...
            if (!(lastDeltaX == 0 && lastDeltaY == 0)) {
                when (currentPipe) {
                    is PipePart.BottomRight -> {
                        val remainingDeltaX = BOTH_CONNECTIONS.minus(currentPipe.horizontalConnections).first()
                        val remainingDeltaY = BOTH_CONNECTIONS.minus(currentPipe.verticalConnections).first()
                        val listToUse = if (lastDeltaX == 1) right else left
                        listToUse += x + remainingDeltaX to y
                        listToUse += x to y + remainingDeltaY
                    }

                    is PipePart.BottomLeft -> {
                        val remainingDeltaX = BOTH_CONNECTIONS.minus(currentPipe.horizontalConnections).first()
                        val remainingDeltaY = BOTH_CONNECTIONS.minus(currentPipe.verticalConnections).first()
                        val listToUse = if (lastDeltaX == -1) left else right
                        listToUse += x + remainingDeltaX to y
                        listToUse += x to y + remainingDeltaY
                    }
                    ...
                    is PipePart.Horizontal -> {
                        val negativeList = if (lastDeltaX == 1) left else right
                        val positiveList = if (lastDeltaX == 1) right else left
                        negativeList += x to y - 1
                        positiveList += x to y + 1
                    }

                    is PipePart.Vertical -> {
                        val negativeList = if (lastDeltaY == 1) right else left
                        val positiveList = if (lastDeltaY == 1) left else right
                        negativeList += x - 1 to y
                        positiveList += x + 1 to y
                    }
                }
            }
        ...
        left.removeAll(mainLoop)
        right.removeAll(mainLoop)

        left.removeAll { it.second !in data.indices || it.first !in data[0].indices }
        right.removeAll { it.second !in data.indices || it.first !in data[0].indices }

        expandGaps(left, mainLoop)
        expandGaps(right, mainLoop)

//        print(mainLoop, left)
//        print(mainLoop, right)

        return listOf(left, right).minBy { it.size }.size.toString()
    }

    private fun expandGaps(area: MutableSet<Pair<Int, Int>>, mainLoop: MutableSet<Pair<Int, Int>>) {
        var lastSize = -1
        while (lastSize != area.size) {
            lastSize = area.size
            val new = mutableSetOf<Pair<Int, Int>>()
            area.forEach {
                for (y in (it.second - 1)..(it.second + 1)) {
                    for (x in (it.first - 1)..(it.first + 1)) {
                        val point = data.getOrNull(y)?.getOrNull(x)
                        if (point != null && !mainLoop.contains(x to y)) {
                            new += x to y
                        }
                    }
                }
    ...
    private fun print(mainLoop: Set<Pair<Int, Int>>, area: Set<Pair<Int, Int>>) {
        data.mapIndexed { y: Int, s: String ->
            s.mapIndexed { x: Int, c: Char ->
                print(
                    if (area.contains(x to y)) {
                        '*'
                    } else {
                        if (mainLoop.contains(x to y)) c else '.'
                    }
                )
            }
            println()
...
L7LLLL--7L-JJL.LL--.|-F-L-|-7-LJJ-F.LJ--LJFJ--JL---7-LJLL-7.|JJ|.LF-FJ-LF|-7-|-J-F|-LLLL|-JLLFJLJ.L|JLJ7-F-FJ-7-J.-J-LJ7JL7-J.J7JJ|J|L-|.7-|
    ...
    private fun Char.toPipePart(x: Int, y: Int): PipePart? {
        return when (this) {
            '|' -> PipePart.Vertical()
            '-' -> PipePart.Horizontal()
            'L' -> PipePart.BottomLeft()
            'J' -> PipePart.BottomRight()
            '7' -> PipePart.TopRight()
            'F' -> PipePart.TopLeft()
            'S' -> {
                val top = data.getOrNull(y - 1)?.getOrNull(x)?.toPipePart(-1, -1)
                val bottom = data.getOrNull(y + 1)?.getOrNull(x)?.toPipePart(-1, -1)
                val left = data.getOrNull(y)?.getOrNull(x - 1)?.toPipePart(-1, -1)
                val right = data.getOrNull(y)?.getOrNull(x + 1)?.toPipePart(-1, -1)

                val verticalConnections = setOfNotNull(
                    if (top?.verticalConnections?.contains(1) == true) -1 else null,
                    if (bottom?.verticalConnections?.contains(-1) == true) 1 else null
                )

                val horizontalConnections = setOfNotNull(
                    if (left?.horizontalConnections?.contains(1) == true) -1 else null,
                    if (right?.horizontalConnections?.contains(-1) == true) 1 else null
                )

                return listOf(
                    PipePart.Vertical(isStart = true),
                    PipePart.Horizontal(isStart = true),
                    PipePart.BottomLeft(isStart = true),
                    PipePart.BottomRight(isStart = true),
                    PipePart.TopRight(isStart = true),
                    PipePart.TopLeft(isStart = true)
                ).find { it.verticalConnections == verticalConnections && it.horizontalConnections == horizontalConnections }
            }
    ...
    sealed interface PipePart {

        val verticalConnections: Set<Int>
        val horizontalConnections: Set<Int>
        val isStart: Boolean

        data class Vertical(
            override val verticalConnections: Set<Int> = setOf(1, -1),
            override val horizontalConnections: Set<Int> = setOf(),
            override val isStart: Boolean = false
        ) : PipePart

        data class Horizontal(
            override val verticalConnections: Set<Int> = setOf(),
            override val horizontalConnections: Set<Int> = setOf(1, -1),
            override val isStart: Boolean = false
        ) : PipePart

        data class BottomLeft(
            override val verticalConnections: Set<Int> = setOf(-1),
            override val horizontalConnections: Set<Int> = setOf(1),
            override val isStart: Boolean = false
        ) : PipePart
