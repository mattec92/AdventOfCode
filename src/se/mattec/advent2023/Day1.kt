...
object Day1 {
    ...
    fun problem1(): String {
        val calibrationValues = data
            .map {
                "" + it.first { it.isDigit() } + it.last { it.isDigit() }
            }.map { it.toInt() }
            ...
            .map { input ->
                val indexOfDigits = listOf(
                    ...
                    .flatMapIndexed { index, digit ->
                        listOf(
                            index to input.indexOf(digit),
                            index to input.lastIndexOf(digit)
                        )
                    }
                    .filter { it.second != -1 }
                val indexOfWords = listOf(
                    "zero",
                    "one",
                    "two",
                    "three",
                ...
                val first = listOfNotNull(
                    indexOfDigits.takeIf { it.isNotEmpty() }?.minBy { it.second },
                    indexOfWords.takeIf { it.isNotEmpty() }?.minBy { it.second }
                ).minBy { it.second }.first
                val last = listOfNotNull(
                    indexOfDigits.takeIf { it.isNotEmpty() }?.maxBy { it.second },
                    indexOfWords.takeIf { it.isNotEmpty() }?.maxBy { it.second }
