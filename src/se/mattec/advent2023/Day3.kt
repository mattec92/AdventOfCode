...
object Day3 {
    ...
    fun problem1(): String {
        ...
        data.indices.forEach { rowIndex ->
            var part = ""
            var adjacent = false
            data[rowIndex].indices.forEach { colIndex ->
                val char = data[rowIndex][colIndex]
                if (char.isDigit()) {
                    part += char
                    adjacent = adjacent || isAdjacent(rowIndex, colIndex)
                } else {
                    if (part.isNotBlank() && adjacent) {
                        parts += part
                    }
                    part = ""
                    adjacent = false
        ...
        return parts.sumOf { it.toInt() }.toString()
    }

    private fun isAdjacent(rowIndex: Int, colIndex: Int): Boolean {
        val fromRow = max(0, rowIndex - 1)
        val toRow = min(data.size - 1, rowIndex + 1)
        val fromCol = max(0, colIndex - 1)
        val toCol = min(data[0].length - 1, colIndex + 1)
    ...
    fun problem2(): String {
        val parts = mutableListOf<Pair<Pair<Int, Int>, Int>>()
        data.indices.forEach { rowIndex ->
            var part = ""
            var adjacent: Pair<Int, Int>? = null
            data[rowIndex].indices.forEach { colIndex ->
                val char = data[rowIndex][colIndex]
                if (char.isDigit()) {
                    part += char
                    adjacent = adjacent ?: findAdjacentGear(rowIndex, colIndex)
                } else {
                    if (part.isNotBlank() && adjacent != null) {
                        parts += adjacent!! to part.toInt()
                    }
                    part = ""
                    adjacent = null
                }
    ...
    private fun findAdjacentGear(rowIndex: Int, colIndex: Int): Pair<Int, Int>? {
        val fromRow = max(0, rowIndex - 1)
        val toRow = min(data.size - 1, rowIndex + 1)
        val fromCol = max(0, colIndex - 1)
        val toCol = min(data[0].length - 1, colIndex + 1)
