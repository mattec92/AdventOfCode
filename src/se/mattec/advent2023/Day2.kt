...
object Day2 {
    ...
    fun problem1(): String {
        return data.mapIndexed { index, rounds ->
            if (rounds.all { group -> group.all { isPossible(it[0].toInt(), it[1]) } }) {
                index + 1
            } else {
                0
            }
    ...
    private fun isPossible(value: Int, color: String): Boolean {
        return when (color) {
            "red" -> value <= 12
            "green" -> value <= 13
            "blue" -> value <= 14
    ...
    fun problem2(): String {
        return data.map { game ->
            ...
            game.forEach { round ->
                round.forEach { group ->
                    val color = group[1]
                    val value = group[0].toInt()
                    val max = maxes[color] ?: 0
                    if (value > max) {
                        maxes[color] = value
                    }
                }
