...
object Day3 {
    ...
    fun problem1(): String {
        val intersectingItems = data
            .map { it.take(it.size / 2).toSet() to it.takeLast(it.size / 2).toSet() }
            .flatMap { it.first.intersect(it.second) }
        return intersectingItems.sumOf { priority(it) }.toString()
    }
    ...
    fun problem2(): String {
        val intersectingItems = data.chunked(3)
