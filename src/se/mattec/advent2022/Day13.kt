...
object Day13 {
    ...
    fun problem1(): String {
        return data.mapIndexed { index: Int, containers: List<Item.Container> ->
    ...
    private fun compare(left: Item, right: Item): Int {
        if (left is Item.Integer && right is Item.Integer) {
            return left.int.compareTo(right.int)
        } else if (left is Item.Container && right is Item.Container) {
            left.list.forEachIndexed { index, left ->
                val right = right.list.getOrNull(index)
                if (right == null) {
                    return 1
                } else {
                    val compareResult = compare(left, right)
                    if (compareResult != 0) {
            ...
            val leftContainer = (left as? Item.Container) ?: Item.Container(mutableListOf(left))
            val rightContainer = (right as? Item.Container) ?: Item.Container(mutableListOf(right))
            return compare(leftContainer, rightContainer)
        }
    }

    fun problem2(): String {
        val sortedData = data.flatten() + parsePair(
            """
                [[2]]
                [[6]]
            """.trimIndent()
        ...
        return ((sortedData.indexOfFirst { it.toString() == "[[2]]" } + 1) *
                (sortedData.indexOfFirst { it.toString() == "[[6]]" } + 1)).toString()
    }
...
[[[],5,[],5],[8,3,4,6,7],[1,[]]]
    ...
    private fun parsePair(it: String): List<Item.Container> {
        val pair = it.split("\n")

        return pair.map { characters ->
            val containerStack = mutableListOf<Item.Container>()
            val intStack = mutableListOf<Int>()

            characters.forEach { char ->
                when (char) {
                    '[' -> {
                        val newContainer = Item.Container()
                        containerStack.lastOrNull()?.let { it.list += newContainer }
                        containerStack += newContainer
                    }

                    ']' -> {
                        containerStack.lastOrNull()?.let {
                            if (intStack.isNotEmpty()) {
                                it.list += Item.Integer(intStack.joinToString("").toInt())
                                intStack.clear()
                            }
                        }
                        val removedContainer = containerStack.removeLast()
                        if (containerStack.isEmpty()) {
                            return@map removedContainer
                        }
                    }
                    ...
                    ',' -> {
                        ...
                        intStack += char.toString().toInt()
                    }
                }
            }

            throw IllegalStateException("Shouldn't get here if input is balanced.")
        }
    }

    sealed interface Item : Comparable<Item> {
        data class Integer(val int: Int) : Item {
            override fun toString() = int.toString()
        }

        data class Container(val list: MutableList<Item> = mutableListOf()) : Item {
            override fun toString() = "[${list.map { it }.joinToString(",")}]"
        }
