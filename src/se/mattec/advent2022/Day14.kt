...
object Day14 {
    ...
    fun problem1(): String {
        val maxX = data.flatten().maxBy { it.first }.first
        val maxY = data.flatten().maxBy { it.second }.second
        ...
        data.forEach { line ->
            ...
            line.forEach { point ->
                ...
                val minX = min(prevPoint!!.first, point.first)
                val maxX = max(prevPoint!!.first, point.first)
                val minY = min(prevPoint!!.second, point.second)
                val maxY = max(prevPoint!!.second, point.second)
                for (x in minX..maxX) {
                    grid[minY][x] = 1
                }
                for (y in minY..maxY) {
        ...
        var sandPlacement = -2 to -2
        var lastPlacement = -1 to -1
        outer@ while (sandPlacement != lastPlacement) {
            var sandX = 500
            var sandY = 0
            ...
            inner@ while (true) {
                if (sandY >= maxY) {
                    ...
                    grid[sandY][sandX] = 2
                    lastPlacement = sandPlacement
                    sandPlacement = sandY to sandX
                    break@inner
                }
            }
        }

        return grid.flatMap { it.toList() }.count { it == 2 }.toString()
    }

    fun problem2(): String {
        val maxY = data.flatten().maxBy { it.second }.second + 2
        val data = data + listOf(listOf(0 to maxY, 1000 to maxY))
        val maxX = data.flatten().maxBy { it.first }.first
        val grid = Array(maxY + 1) { IntArray(maxX + 1) { 0 } }
    ...
    private fun print(grid: Array<IntArray>) {
        val minX = grid.map { it.indexOfFirst { it != 0 } }.filter { it != -1 }.min()
        val maxX = grid.map { it.indexOfLast { it != 0 } }.filter { it != -1 }.max()
        grid.forEach { row ->
            println(
                row.toList()
                    .subList(minX, maxX)
