...
object Day4 {
    ...
    fun problem1(): Int {
        val guardsBySleepTime = getShifts()
                .groupBy { it.guardNr }
                .mapValues { it.value.flatMap { it.minutesAsleep } } //Get a map of guardNr -> minutesAsleep

        val guardWithMostSleepTime = guardsBySleepTime.maxBy { it.value.size }!!

        val mostCommonSleepTime = guardWithMostSleepTime.value.groupBy { it } //Group by minute to find the most common minute
                .maxBy { it.value.size }!! //Find the largest group
                .key

        return guardWithMostSleepTime.key.toInt() * mostCommonSleepTime
    }

    fun problem2(): Int {
        val guardWithMostCommonSleepTime = getShifts()
                .groupBy { it.guardNr }
                .mapValues { it.value.flatMap { it.minutesAsleep } } //Get a map of guardNr -> minutesAsleep
                .mapValues { it.value.groupBy { it }.maxBy { it.value.size } } //Group values by minute and then only keep the most common
                .maxBy { it.value?.value?.size ?: 0 }!! //Find the guard with the most common sleep time

        return guardWithMostCommonSleepTime.key.toInt() * guardWithMostCommonSleepTime.value!!.key
    }

    private val newShiftPattern = Pattern.compile(".* #(\\d*).*")
    private val sleepPattern = Pattern.compile(".*:(\\d*)] falls asleep")
    private val wakePattern = Pattern.compile(".*:(\\d*)] wakes up")

    private fun getShifts(): List<Shift> {
        val shifts = mutableListOf<Shift>()

        var guardNr: String? = null
        var sleepTime: Int? = null
        var minutesAsleep: MutableList<Int>? = null
        for (entry in data) {
            val newShiftMatcher = newShiftPattern.matcher(entry)
            if (newShiftMatcher.matches()) {
                if (guardNr != null && minutesAsleep != null) {
                    shifts += Shift(guardNr, minutesAsleep)
                }
                guardNr = newShiftMatcher.group(1)
                minutesAsleep = mutableListOf()
                continue
            }
            val sleepMatcher = sleepPattern.matcher(entry)
            if (sleepMatcher.matches()) {
                sleepTime = sleepMatcher.group(1).toInt()
                continue
            }
            val wakeMatcher = wakePattern.matcher(entry)
            if (wakeMatcher.matches()) {
                val wakeTime = wakeMatcher.group(1).toInt()
                minutesAsleep?.apply { for (i in sleepTime!! until wakeTime) add(i) }
            }
        }

        //Last shift will not have a "next shift" that triggers adding
        if (guardNr != null && minutesAsleep != null) {
            shifts += Shift(guardNr, minutesAsleep)
        }
    ...
    private data class Shift(val guardNr: String, val minutesAsleep: List<Int>)
